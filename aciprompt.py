...
def main():
    ...
    c1_resp = tenant.push_to_apic(session)
    c2_resp = tenant.push_to_apic(session)

    if c1_resp.ok:
        print('Web-to-App contract created successfully !!!')

    if not c1_resp.ok:
        print('Error: Could not create Web-to-App contract')
        print(c1_resp.status_code)




    if c2_resp.ok:
        print('App-to-Data contract created successfully !!!')

    if not c2_resp.ok:
        print('Error: Could not create App-to-Data contract')
        print(c2_resp.status_code)
